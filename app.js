(() => {
  ...
  const elements = {
    ...
    reverseOrderButton: document.querySelector("#reverseOrderButton"),
    ...
    scanApplyButton: document.querySelector("#scanApplyButton"),
    scanAutoButton: document.querySelector("#scanAutoButton"),
    scanCancelButton: document.querySelector("#scanCancelButton"),
    scanCanvas: document.querySelector("#scanCanvas"),
    scanCloseButton: document.querySelector("#scanCloseButton"),
    scanDialog: document.querySelector("#scanDialog"),
    scanFileName: document.querySelector("#scanFileName"),
    scanOverlay: document.querySelector("#scanOverlay"),
    scanResetButton: document.querySelector("#scanResetButton"),
  ...
  function render() {
    ...
    elements.savePdfButton.disabled = state.busy || !state.lastPdf;
    elements.resultCard.hidden = !state.lastPdf;
    if (state.lastPdf) {
      elements.resultMeta.textContent = state.lastPdf.fileName + " / " + bytesLabel(state.lastPdf.blob.size);
    }
    ...
    elements.scanSelectedButton.disabled = state.busy || !selected;
    if (selected) {
  ...
  function moveItem(fromIndex, toIndex) {
    ...
    invalidatePdf();
    elements.orderMode.value = "manual";
  ...
  async function addFiles(fileList, options = {}) {
    ...
    const nextItems = files.map((file) => ({
      ...
      scanLabel: "未スキャン",
      scanMode: "off",
      addedOrder: state.nextOrder++,
    }));

    invalidatePdf();
    state.items.push(...nextItems);
    state.selectedId = nextItems[0].id;
    if (elements.orderMode.value !== "import" && elements.orderMode.value !== "manual") {
      reorderItems(elements.orderMode.value, false);
    }
    render();
    setStatus(`${formatter.format(nextItems.length)}枚追加しました`);

    if (options.openScanner) await openScanEditor(nextItems[0].id);
  }

  function naturalFileNameCompare(leftItem, rightItem) {
    const leftName = leftItem.originalFile.name || "";
    const rightName = rightItem.originalFile.name || "";
    return leftName.localeCompare(rightName, "ja", { numeric: true, sensitivity: "base" });
  }

  function reorderItems(mode = elements.orderMode.value, announce = true) {
    if (!state.items.length) return;
    const selectedId = state.selectedId;
    const compare = (leftItem, rightItem) => {
      if (mode === "name") {
        return naturalFileNameCompare(leftItem, rightItem) || (leftItem.addedOrder - rightItem.addedOrder);
      }
      if (mode === "date-desc" || mode === "date-asc") {
        const leftDate = Number(leftItem.originalFile.lastModified) || 0;
        const rightDate = Number(rightItem.originalFile.lastModified) || 0;
        const dateDifference = mode === "date-desc" ? rightDate - leftDate : leftDate - rightDate;
        return dateDifference || naturalFileNameCompare(leftItem, rightItem) || (leftItem.addedOrder - rightItem.addedOrder);
      }
      if (mode === "import") {
        return leftItem.addedOrder - rightItem.addedOrder;
      }
      return 0;
    };
    ...
    if (announce) {
      const labels = {
        ...
        manual: "現在の順番",
        name: "自然なファイル名順",
        "date-desc": "ファイル日時の新しい順",
        "date-asc": "ファイル日時の古い順",
      };
      setStatus("並び順を" + (labels[mode] || "現在の順番") + "にしました");
    }
  }
  ...
  async function applyScanToAll() {
    ...
    try {
      for (let index = 0; index < state.items.length; index += 1) {
        setStatus(`${formatter.format(index + 1)} / ${formatter.format(state.items.length)} を自動スキャン中`);
        await applyScanToItem(state.items[index], "scan");
      }
      setStatus("自動スキャンを適用しました");
    } catch (error) {
      console.error(error);
      const message = error instanceof Error ? error.message : String(error);
      setStatus(`スキャンできませんでした: ${message}`);
    } finally {
      state.busy = false;
      render();
    }
  ...
  async function applyScanToItem(item, scanMode) {
    ...
    try {
      ...
      setItemFromBlob(item, result.blob, result.label, scanMode);
    } catch (error) {
      console.warn(error);
      resetItemToOriginal(item, "スキャン失敗");
    }
  }

  function resetItemToOriginal(item, label = "未スキャン") {
    invalidatePdf();
    if (item.previewUrl) URL.revokeObjectURL(item.previewUrl);
    item.file = item.originalFile;
    item.previewUrl = URL.createObjectURL(item.file);
    item.scanLabel = label;
    item.scanMode = "off";
  }

  function setItemFromBlob(item, blob, label, scanMode) {
    invalidatePdf();
    if (item.previewUrl) URL.revokeObjectURL(item.previewUrl);
    item.file = makeProcessedFile(blob, item.originalFile);
    item.previewUrl = URL.createObjectURL(item.file);
    item.scanLabel = label;
  ...
  async function processScanImage(file, scanMode) {
    ...
    let label = scanMode === "scan" ? "自動スキャン" : "文字くっきり";
  ...
  function syncDialogLock() {
    const dialogOpen = !elements.addDialog.hidden || !elements.scanDialog.hidden;
    document.body.classList.toggle("is-dialog-open", dialogOpen);
  }

  async function openScanEditor(itemId) {
    if (state.busy) return;
    ...
    state.busy = true;
    state.selectedId = item.id;
    setStatus("スキャン画面を開いています");
    render();

    try {
      const canvas = await fileToCanvas(item.originalFile, SCAN_OUTPUT_EDGE);
      state.scan.itemId = item.id;
      state.scan.sourceCanvas = canvas;
      state.scan.corners = detectInitialScanCorners(canvas);
      elements.scanFileName.textContent = item.originalFile.name;
      drawScanCanvas();
      elements.scanDialog.hidden = false;
      syncDialogLock();
      renderScanOverlay();
      setStatus("青い枠を合わせてください");
    } catch (error) {
      console.error(error);
      const message = error instanceof Error ? error.message : String(error);
      setStatus(`スキャン画面を開けませんでした: ${message}`);
      clearScanState();
    } finally {
      state.busy = false;
      setScanDialogBusy(false);
      render();
    }
  }

  function closeScanEditor(force = false) {
    if (state.busy && !force) return;
    elements.scanDialog.hidden = true;
    clearScanState();
    syncDialogLock();
  }
  ...
  function clearScanState() {
    state.scan.corners = [];
    state.scan.draggingIndex = null;
    state.scan.itemId = null;
    state.scan.sourceCanvas = null;
    elements.scanFileName.textContent = "未選択";
    elements.scanOverlay.replaceChildren();
    const context = elements.scanCanvas.getContext("2d");
    if (context) context.clearRect(0, 0, elements.scanCanvas.width, elements.scanCanvas.height);
    elements.scanCanvas.width = 0;
    elements.scanCanvas.height = 0;
  }

  function drawScanCanvas() {
    const canvas = state.scan.sourceCanvas;
    if (!canvas) return;
    elements.scanCanvas.width = canvas.width;
    elements.scanCanvas.height = canvas.height;
    const context = elements.scanCanvas.getContext("2d", { alpha: false });
    if (!context) return;
    context.drawImage(canvas, 0, 0);
  }

  function detectInitialScanCorners(canvas) {
    const region = detectPaperRegion(canvas);
    if (region?.corners) return clampCorners(region.corners, canvas);
    if (region?.bbox) return rectToCorners(region.bbox, canvas);
    return defaultScanCorners(canvas);
  }

  function defaultScanCorners(canvas) {
    const paddingX = Math.max(1, canvas.width * 0.045);
    const paddingY = Math.max(1, canvas.height * 0.045);
    return [
      { x: paddingX, y: paddingY },
      { x: canvas.width - paddingX, y: paddingY },
      { x: canvas.width - paddingX, y: canvas.height - paddingY },
      { x: paddingX, y: canvas.height - paddingY },
    ];
  }

  function rectToCorners(rect, canvas) {
    return clampCorners([
      { x: rect.x, y: rect.y },
  ...
  function clampCorners(corners, canvas) {
    return corners.map((point) => ({
      x: clamp(point.x, 0, canvas.width - 1),
      y: clamp(point.y, 0, canvas.height - 1),
    }));
  }
  ...
  function renderScanOverlay() {
    ...
    elements.scanOverlay.setAttribute("viewBox", `0 0 ${canvas.width} ${canvas.height}`);
    elements.scanOverlay.setAttribute("width", String(canvas.width));
    elements.scanOverlay.setAttribute("height", String(canvas.height));

    const polygon = document.createElementNS(SVG_NS, "polygon");
    polygon.setAttribute("class", "scan-selection");
    polygon.setAttribute("points", scanPoints(corners));
    elements.scanOverlay.append(polygon);

    const handleRadius = Math.max(18, Math.min(canvas.width, canvas.height) * 0.018);
    corners.forEach((point, index) => {
      const hitArea = document.createElementNS(SVG_NS, "circle");
      hitArea.setAttribute("class", "scan-hit-area");
      hitArea.setAttribute("cx", String(point.x));
      hitArea.setAttribute("cy", String(point.y));
      hitArea.setAttribute("r", String(handleRadius * 2.1));
      hitArea.dataset.index = String(index);
      hitArea.addEventListener("pointerdown", startScanCornerDrag);

      const handle = document.createElementNS(SVG_NS, "circle");
      handle.setAttribute("class", "scan-handle");
      handle.setAttribute("cx", String(point.x));
      handle.setAttribute("cy", String(point.y));
      handle.setAttribute("r", String(handleRadius));
      handle.dataset.index = String(index);
      handle.addEventListener("pointerdown", startScanCornerDrag);

      elements.scanOverlay.append(hitArea, handle);
    });
  }

  function scanPoints(corners) {
    return corners.map((point) => `${point.x},${point.y}`).join(" ");
  }

  function startScanCornerDrag(event) {
    if (!state.scan.sourceCanvas || state.busy) return;
    event.preventDefault();
    const index = Number(event.currentTarget.dataset.index);
    if (!Number.isInteger(index)) return;
    state.scan.draggingIndex = index;
    updateScanCorner(event);

    const handleMove = (moveEvent) => updateScanCorner(moveEvent);
    const stopDrag = () => {
      state.scan.draggingIndex = null;
  ...
  function updateScanCorner(event) {
    const index = state.scan.draggingIndex;
    if (!Number.isInteger(index) || !state.scan.sourceCanvas) return;
    state.scan.corners[index] = scanPointFromEvent(event);
    renderScanOverlay();
  }

  function scanPointFromEvent(event) {
    const canvas = state.scan.sourceCanvas;
    const rect = elements.scanOverlay.getBoundingClientRect();
    const width = Math.max(1, rect.width);
    const height = Math.max(1, rect.height);
    return {
      x: clamp((event.clientX - rect.left) * (canvas.width / width), 0, canvas.width - 1),
      y: clamp((event.clientY - rect.top) * (canvas.height / height), 0, canvas.height - 1),
    };
  }

  function autoDetectScanArea() {
    const canvas = state.scan.sourceCanvas;
    if (!canvas || state.busy) return;
    state.scan.corners = detectInitialScanCorners(canvas);
    renderScanOverlay();
    setStatus("範囲を自動検出しました");
  }

  function resetScanArea() {
    const canvas = state.scan.sourceCanvas;
    if (!canvas || state.busy) return;
    state.scan.corners = defaultScanCorners(canvas);
    renderScanOverlay();
    setStatus("全体を選択しました");
  }

  async function applyManualScan() {
    const item = state.items.find((entry) => entry.id === state.scan.itemId);
    const sourceCanvas = state.scan.sourceCanvas;
    if (!item || !sourceCanvas || state.busy) return;

    const corners = clampCorners(state.scan.corners, sourceCanvas);
    if (!isUsableQuad(corners, sourceCanvas.width, sourceCanvas.height)) {
      setStatus("青い枠をもう少し広げてください");
      return;
    }

    state.busy = true;
    setScanDialogBusy(true);
    render();
    setStatus("スキャン中");

    try {
      const outputCanvas = warpCanvasToDocument(sourceCanvas, corners);
      softenEdgeObstructions(outputCanvas);
      enhanceScanCanvas(outputCanvas);
      const blob = await canvasToBlob(outputCanvas, SCAN_OUTPUT_QUALITY);
      setItemFromBlob(item, blob, "スキャン済み", "scan");
      state.selectedId = item.id;
      closeScanEditor(true);
      setStatus("スキャンしました");
    } catch (error) {
      console.error(error);
      const message = error instanceof Error ? error.message : String(error);
  ...
  function setScanDialogBusy(isBusy) {
    [
      elements.scanApplyButton,
      elements.scanAutoButton,
      elements.scanCancelButton,
      elements.scanCloseButton,
      elements.scanResetButton,
    ].forEach((button) => {
  ...
  async function createPdf() {
    ...
    try {
      ...
      state.lastPdf = { blob, fileName };
      elements.fileName.value = fileName;
      render();
      const saveMode = await savePdf(blob, fileName);
      if (saveMode === "shared") {
        setStatus(fileName + " を共有・保存できます");
      } else if (saveMode === "saved") {
        setStatus(fileName + " を保存しました");
      } else if (saveMode === "cancelled") {
        setStatus(fileName + " を準備しました（下のボタンから保存できます）");
      } else {
  ...
  async function saveLastPdf() {
    if (!state.lastPdf || state.busy) return;
    const { blob, fileName } = state.lastPdf;
    try {
      const saveMode = await savePdf(blob, fileName);
      if (saveMode === "shared") {
        setStatus(fileName + " を共有・保存できます");
        ...
        setStatus(fileName + " を準備したままにしています");
      } else {
        setStatus(fileName + " をダウンロードしました");
      }
    } catch (error) {
  ...
  elements.addCloseButton.addEventListener("click", closeAddDialog);
  ...
  elements.scanCaptureButton.addEventListener("click", () => {
    closeAddDialog();
    elements.scanInput.click();
  });
  elements.scanInput.addEventListener("change", (event) => {
    void addFiles(event.target.files, { openScanner: true });
    elements.scanInput.value = "";
  });
  elements.scanSelectedButton.addEventListener("click", () => {
    if (state.selectedId) void openScanEditor(state.selectedId);
  });
  elements.scanApplyButton.addEventListener("click", () => {
    void applyManualScan();
  });
  elements.scanAutoButton.addEventListener("click", autoDetectScanArea);
  elements.scanCancelButton.addEventListener("click", () => closeScanEditor());
  elements.scanCloseButton.addEventListener("click", () => closeScanEditor());
  elements.scanResetButton.addEventListener("click", resetScanArea);
  ...
  elements.orderMode.addEventListener("change", () => {
    reorderItems(elements.orderMode.value);
  });
  elements.reverseOrderButton.addEventListener("click", reverseOrder);
  elements.quality.addEventListener("input", () => {
    invalidatePdf();
    render();
  });
  ["pageSize", "margin", "fitMode", "maxEdge"].forEach((key) => {
  ...
  window.addEventListener("keydown", (event) => {
    ...
    if (!elements.scanDialog.hidden) {
      closeScanEditor();
    } else if (!elements.addDialog.hidden) {
      closeAddDialog();
    }
  });
